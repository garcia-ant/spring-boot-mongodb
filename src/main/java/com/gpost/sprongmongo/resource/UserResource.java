...
public class UserResource {
	...
	public ResponseEntity<Void> update(@RequestBody UserDTO objdto, @PathVariable String id) {
	    User obj = service.fromDTO(objdto);
	    obj.setId(id);  // Set the ID from the path variable
	    obj = service.update(obj);  // Call the service to update the user
	    return ResponseEntity.noContent().build();  // Return a no content status (204)
	}
